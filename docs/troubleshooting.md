...
**Symptoms:**
- Zed logs (`zed --foreground`) show a line starting with `depsy: could not install depsy-lsp vX.Y.Z`
- Features from the latest release are missing

**Cause:**

The extension could not download the language-server release matching its own version (no access to `github.com`, or a checksum mismatch) and started the newest release already on disk instead.

**Solutions:**

1. Restore access to `https://github.com` and restart Zed: the download is retried at every start
